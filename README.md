...
| 模块名称 | 定位 | 核心技术点 | 
| :---- | :---- | :---- | :---- |
| **membin** | 高性能二进制缓冲区 | SSO, 引用计数, COW (写时复制) | 
| **protobuf** | 轻量级协议编解码 | 零拷贝视图 (std::string\_view) | 
| **json** | 很简单的实现，性能一定不佳，不建议引用 | std::variant 递归结构 | 
...
// 1\. 自动根据长度切换 SSO 或堆分配  
membin data;   
data.push\_back(0x01); // 此时位于 SSO 栈区

// 2\. COW 特性：拷贝时仅增加引用计数，不触发底层内存复制  
membin copy \= data; 

// 3\. 性能友好的 API  
auto sv \= data.view(); // 提供 string\_view 接口，零拷贝访问底层数据
